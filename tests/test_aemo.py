	...
	def test_studycase_fs_result_export(self):
		...
		test_export_file = os.path.join(
			test_export_pth, '{}{}{}{}{}.csv'.format(
				...
				pscharmonics.constants.Results.joiner, pscharmonics.constants.Contingencies.intact,
			)
		)
...
@unittest.skipUnless(include_slow_tests, 'Tests that require initialising PowerFactory have been skipped')
class TestPFProjectContingencyLines(unittest.TestCase):
	"""
		Tests creation of contingency cases when using lines rather than outages
	"""
	# Names used for testing contingencies (convergent and non-convergent)
	test_line = 'TEST Line'
	test_line_nc = 'TEST Line NC'
	# This line switches in some circuits to confirm that method works as well
	test_line_switch_in = 'TEST Line Switch In'
	...
	def test_pre_case_check_single_study_case(self):
		...
		# Confirm the contents of a single value matches expectations
		c = pscharmonics.constants.Contingencies
		sc_result = df.loc[(self.test_name, self.test_line), :]
		self.assertEqual(sc_result[c.cont], self.test_line)
		self.assertEqual(sc_result[c.status], True)

		# Confirm the non-convergent case is as expected
		c = pscharmonics.constants.Contingencies
		sc_result = df.loc[(self.test_name, self.test_line_nc), :]
		self.assertEqual(sc_result[c.cont], self.test_line_nc)
		self.assertEqual(sc_result[c.status], False)

		# Confirm the switch line in case is convergent
		c = pscharmonics.constants.Contingencies
		sc_result = df.loc[(self.test_name, self.test_line_switch_in), :]
		self.assertEqual(sc_result[c.cont], self.test_line_switch_in)
		self.assertEqual(sc_result[c.status], True)
	...
	def test_pre_case_check_multiple_study_cases(self):
		"""
			Test that when the pre-case check is run for the two study cases
			the dataframe that is returned matches what is expected.  In this case the analysis
			is done considering the outages taking place on the lines.
		:return None:
		"""
		# Test Names (detailed in Inputs spreadsheet)
		test_case1 = 'BASE'
		...
		# Since project is duplicated expect results to be the same in both cases
		c = pscharmonics.constants.Contingencies
		sc_result = df.loc[(test_case1, self.test_line), :]
		self.assertEqual(sc_result[c.cont], self.test_line)
		self.assertEqual(sc_result[c.status], True)

		# Confirm the non-convergent case is as expected
		c = pscharmonics.constants.Contingencies
		sc_result = df.loc[(test_case2, self.test_line), :]
		self.assertEqual(sc_result[c.cont], self.test_line)
		self.assertEqual(sc_result[c.status], True)
	...
	def test_case_creation_lines_only(self):
		"""
			Test that when the pre-case check is run for the two study cases
			the dataframe that is returned matches what is expected.

			This test is carried out with outages only being taken on the lines with no outages defined
			using the switches
		:return None:
		"""
		# Adjust self.setting.contingencies to not have any contingencies that relate to circuit outages
		updated_cont_details = dict()
		for cont_name, cont_values in self.settings.contingencies.items():
			# The first level contains the name of the contingency and all the CB / line outages associated
			updated_cont_details[cont_name] = {
				outage_type: outage_details for outage_type, outage_details in cont_values.items()
				if outage_type is not pscharmonics.constants.Contingencies.cb
			}
		...
		# Carry out pre-case check using fault cases on what should be two different study_cases
		df = pf_project.pre_case_check(contingencies=updated_cont_details)
...
class TestPFDetailedInputs(unittest.TestCase):
	...
	def test_complete_test_produce_outputs_4_small_step_size(self):
		...
		"""
		# Create path for results from detailed tests
		target_pth = os.path.join(TESTS_DIR, 'Detailed_4')
		if os.path.isdir(target_pth):
			print('Existing contents in path: {} will be deleted'.format(target_pth))
			files = glob.glob(target_pth + '\\*')
			for f in files:
		...
		# Import settings for Detailed Study
		settings_file = os.path.join(TESTS_DIR, 'Inputs_Detailed4.xlsx')
		inputs = pscharmonics.file_io.StudyInputs(pth_file=settings_file)
	...
	def test_complete_test_produce_outputs_5_lines_only(self):
		"""
			...
			This test is run using the outputs which consist of some line outages on the
		:return:
		"""
		# Create path for results from detailed tests
		target_pth = os.path.join(TESTS_DIR, 'Detailed_5')
		if os.path.isdir(target_pth):
			print('Existing contents in path: {} will be deleted'.format(target_pth))
			files = glob.glob(target_pth + '\\*')
			for f in files:
		...
		# Import settings for Detailed Study
		settings_file = os.path.join(TESTS_DIR, 'Inputs_Detailed5.xlsx')
		inputs = pscharmonics.file_io.StudyInputs(pth_file=settings_file)
...
class TestPFProjectContingencyLines(unittest.TestCase):
	"""
		Tests creation of contingency cases when using lines rather than outages specific to the AEMO model
		Tests are carried out on the latest version of the AEMO model
	"""
	# Names used for testing contingencies (convergent and non-convergent)
	test_line = 'TEST Line'
	test_line_nc = 'TEST Line NC'
